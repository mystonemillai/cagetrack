            ...
            {coachProfile && (
              <div className="rounded-xl bg-navy-light border border-wheat/8 p-6 mb-6">
                <div className="flex items-start justify-between">
                  ...
                  <div className="flex gap-3">
                    const { nativeShare } = await import('@/lib/native'); const result = await nativeShare('CageTrack Coach Profile', 'Check out my coaching profile on CageTrack', `https://cagetrack.com/coach/${coachProfile.id}`); if (result.copied) alert('Profile link copied!'); }} className="text-xs text-wheat hover:underline">Share</button>
