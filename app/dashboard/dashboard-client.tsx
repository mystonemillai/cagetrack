          ...
          <form onSubmit={handleCreatePlayer} className="space-y-5">
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {AGE_GROUPS.map((ag) => (
                  <button key={ag} type="button" onClick={() => setAgeGroup(ag)} className={`px-3 py-1.5 rounded-md text-sm font-medium transition-all ${ageGroup === ag ? 'bg-wheat text-navy' : 'bg-navy-light border border-wheat/10 text-offwhite/50 hover:border-wheat/25'}`}>{ag}</button>
                ))}
              </div>
            </div>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {POSITIONS.map((pos) => (
                  <button key={pos} type="button" onClick={() => togglePosition(pos)} className={`px-3 py-1.5 rounded-md text-sm font-medium transition-all ${selectedPositions.includes(pos) ? 'bg-wheat text-navy' : 'bg-navy-light border border-wheat/10 text-offwhite/50 hover:border-wheat/25'}`}>{pos}</button>
                ))}
              </div>
            </div>
            ...
            {createError && (
              <div className="p-3 rounded-lg bg-red-500/10 border border-red-500/20 text-red-400 text-sm">{createError}</div>
            )}
      ...
      <main className="pt-20 pb-24 px-4 max-w-5xl mx-auto">
        ...
        {!isCoach && hasPlayers && (
          <>
            ...
            {isFamily && (
              <button onClick={() => setShowCreatePlayer(true)} className="mt-6 w-full p-4 rounded-xl border border-dashed border-wheat/15 text-offwhite/30 hover:text-wheat hover:border-wheat/30 transition-all text-sm">
                + Add Another Player
              </button>
            )}
          </>
...
function CoachInviteCodeEntry({ coachProfile }: { coachProfile: any }) {
  ...
  async function handleConnect(e: React.FormEvent) {
    ...
    const { error: updateError } = await supabase
      .from('player_coaches')
      .update({ coach_profile_id: coachProfile.id, status: 'active', connected_at: new Date().toISOString() })
      .eq('id', data.id);
